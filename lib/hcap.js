...
var CAP = function (args) {
  ...
  this.textLen = 6; //默认验证码字数
  this.noiseType = 2; //默认噪声类型
  this.noiseSigma = 1; //默认噪声幅度

  this.is_first = true;

  this._cache_num = 20; //默认缓存20个验证码
  this.buf = []; //缓存数组
...
CAP.prototype._init = function (args) {
  ...
  if (typeof arguments[0] === 'object') {
    ...
    this.textLen = obj.textLen || this.textLen;
...
//设置默认验证码生成函数
CAP.prototype._default_generate = gen_func;

//调用C++的CIMG库的create函数生成验证码文件
CAP.prototype._call_create = function (text, len, j) {
  var that = this;
  var filepath = that.filename.replace('{$number}', j);
  hrobj.create(text, filepath, len, that.width, that.height, that.offset, that.quality, isjpeg, that.fontSize, that.noiseType, that.noiseSigma);
...
//默认方式创建
CAP.prototype._default_create = function () {
  for (var i = 0; i < this._cache_num; i++) {
    this._call_create(this._default_generate(this.textLen), this.textLen, i);
  }
  return this;
}

//自定义创建
CAP.prototype._custom_create = function () {
  for (var i = 0; i < this._cache_num; i++) {
